...
def write_info():
    info_file = 'info.txt'
    f = open(info_file, 'w')
    f.write("----- Some information about computer -----\n")
    f.write("User name: " + os.popen("whoami").read())
    f.write("OS: " + os.popen("uname").read())
    f.write("Computer name: " + os.popen("hostname").read())
    f.write("Path to sys bin files: " + os.popen("find / -name \"sbin\"").read())

    f.write("Keyboard " + os.popen("setxkbmap -query | grep layout").read())
    f.write(
        "Screen height: " + os.popen("xrandr --current | grep '*' | uniq | awk '{print $1}' | cut -d 'x' -f2").read())
    f.write(os.popen("grep MemTotal /proc/meminfo").read())
    f.write("Disk storage: " + os.popen("df | grep '^/dev/[hs]d' | awk '{s+=$2} END {print s/1048576}'").read()[
                               :-1] + " GB\n")
    # print >> f, "md5sum info.txt | awk '{ print $1 }'"
    f.close()
...
def check_hash():
    ##################################################
    data = ''
    with open('info.txt', 'r') as file_to_check:
        data += file_to_check.read()
    md5_returned = hashlib.md5()
    md5_returned.update(data.encode('utf-8'))
    hash_sum = md5_returned.hexdigest()
    ##################################################

    hash_info = open("hash_info.txt", 'r')
    hash_code = hash_info.readline()
    hash_code = hash_code[:-1]
    print 'hash from file = ' + hash_code
    print 'hash from computer = ' + hash_sum
    return hash_code, hash_sum


if __name__ == '__main__':
    key = 'marta2333'
    # key = 'password' + str(random.randint(2, 100))
    file_name = 'data.txt'
...
#########################################################
    ...
    hash_code, hash_sum = check_hash()

    if hash_sum != hash_code:
        print 'hash keys are not equal'
