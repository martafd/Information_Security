...
if __name__ == '__main__':
    ...
    f.write("Screen height: " + os.popen("xrandr --current | grep '*' | uniq | awk '{print $1}' | cut -d 'x' -f2").read())
    f.write(os.popen("grep MemTotal /proc/meminfo").read())
    f.write("Disk storage: " + os.popen("df | grep '^/dev/[hs]d' | awk '{s+=$2} END {print s/1048576}'").read()[:-1] + " GB\n")
